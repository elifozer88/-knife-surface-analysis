...
# Weka J48 budamasını (pessimistic pruning) sklearn üzerinde en iyi simüle eden ccp_alpha parametresidir[cite: 188].
# Klasik unpruned kısıtlamalarından bağımsız, budama katsayısı ile ağacı dinamik olarak budayacağız.
param_grid = {
    "criterion": ["entropy"],            # C4.5'in bilgi kazancı (Information Gain) temeli [cite: 288]
    "ccp_alpha": [0.0, 0.001, 0.005, 0.01, 0.015, 0.02], # Gerçek budanmış (pruned) ağaç etkisi için katsayılar
...
# Makaledeki Spesifikasyon Limitlerine Göre Sınıflar (Sayfa 2 & Sayfa 10) [cite: 43, 324]
class_names = [
    "Class 0 (Ra < 0.11 um - Waste)",   # Alt spesifikasyon sınırı altı fire [cite: 43, 324]
    "Class 1 (0.11 <= Ra <= 0.45 um)", # Kabul edilebilir kaliteli süreç [cite: 43, 324]
    "Class 2 (Ra > 0.45 um - Waste)"    # Üst spesifikasyon sınırı üstü fire [cite: 43, 324]
]
...
# Görseli outputs/figures/ altına kaydet
matrix_path = "outputs/figures/c45_pruned_confusion_matrix.png"
plt.savefig(matrix_path, bbox_inches='tight', dpi=300)
plt.close(fig_cm)
print(f"✓ Confusion Matrix plot saved to '{matrix_path}'")

# 4. EN ÖNEMLİ 5 ÖZELLİK (Information Gain Azalımı ile) [cite: 189]
print("\n" + "=" * 60)
...
plot_tree(
    ...
    filled=True, 
    rounded=True, 
    impurity=True, # Her düğümde Entropy değerini (E = ...) gösterir
    fontsize=10, 
    ax=ax_tree
)
plt.tight_layout()

tree_path = "outputs/figures/c45_pruned_tree.png"
try:
